...
[Далее: наведение телескопа и получение изображений в Dina System](SlewExp.md)
...
(см. контакты ответственных за состояние Цейсс-1000/ПЗС-фотометра в разделе [Дополнительная информация](AddInfo.md)).
...
Для этого нужно последовательно выполнить ряд шагов, описанных ниже.
    ...
    1. На рабочем столе запустить иконку BTA TCS Online (иконка №1), 
      в браузере откроется страница [tb.sao.ru/tcs/main.html](http://tb.sao.ru/tcs/main.html)
...
*Веб-страница с камерами подкупольного пространства телескопа Цейсс-1000, видом в сторону БТА.*
*Также доступны вкладки с метеоусловиями, камерами всего неба и т.д.*
...
2. Запустить на рабочем столе иконку Xming (иконка №2), если это приложение не запущено. В трее должен быть запущен Xming Server.

3. Запустить иконку Run Sjphone on ZTCS (иконка №3).

4. Запустить иконку 3CX Phone (иконка №4), в открывшемся окне выбрать Destination number obs@192.168.3.97, 
...
1. Для включения питания телескопа и купола, открытия забрала и крышек главного зеркала (ГЗ)
  ...
  [ztcs.sao.ru/tcs/zeiss_moxa_new.php](http://ztcs.sao.ru/tcs/zeiss_moxa_new.php)
...
4. Включить кнопки питания стоек и приводов Cabinets power, SEW Power (в итоге должна гореть зелёным линейка из трёх круглых кнопок). 
...
открыть створки забрала кнопкой DomeDoors Open, 
проверить смену статусов забрала в окошке слева, 
после окончания открытия створок забрала должен высветиться статус "Doors Stop 100%". 


**Во время движения купола забрало должно находиться в одном из крайних положений, либо в открытом, либо в закрытом!**

6. Открыть крышки ГЗ кнопкой Mirror Open, проверить статус полностью открытых крышек в окошке слева "Covers Stop 100%".
...
1.  Запустить на рабочем столе иконку Remote Desktop (иконка №7). В открывшемся окне убедиться, 
    что введён адрес zobs.sao.ru, нажать кнопку Подключить. 
...
Если по каким-то причинам наблюдения начались после полуночи, используется вечерняя дата.
